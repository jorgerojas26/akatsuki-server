setTimeout(() => {
    ...
    view_collections_button.onclick = () => {
        ...
        const collections = window.current_task_info.collections;
        ...
        visualize_resource(collections, title, collections_count);
    };
    ...
    view_keywords_button.onclick = () => {
        ...
        visualizer_textarea.style.display = 'none';
        visualizer_modal_editor.style.display = 'block';
        visualize_resource(window.current_task_info.keywords, title);
    };
    ...
    visualizer_save_button.onclick = () => {
        ...
        update_resource(resource_id, { [resource_name]: text });
    ...
    async function get_server_info_for_this_task(job_title_slug) {
        ...
        if (!window.location.href.includes('dashboard')) {
            eval(current_task_guide_info.script);
        }
    ...
    wrappers.forEach((wrapper) => {
        ...
        button.onclick = () => {
            ...
            if (!response.error) {
                ...
                window.current_task_info.collections = response.collections;
            }
        };
...
if (missed_form?.action.includes('/contend')) {
    const appen_head = document.head.cloneNode(true);
    const appen_head_scripts = appen_head.querySelectorAll('script');
    ...
    if (!window.location.href.includes('file://')) post_to_discord(html_blob, correction_job_title);
}
